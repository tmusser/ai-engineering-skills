...
def is_safe_markdown_file(path: Path, root: Path) -> bool:
    """Return True for regular in-root Markdown files that are safe to index."""
...
def iter_markdown_paths(root: Path) -> list[Path]:
    """Return all safe markdown files under root in stable order."""
    root = root.resolve()
    ...
    for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        if current_dir.is_symlink() or is_excluded_path(relative_dir):
            dirnames[:] = []
            continue
        ...
        for name in sorted(dirnames):
            ...
            relative_candidate = relative_dir / name
            if name in EXCLUDED_DIR_NAMES or is_excluded_path(relative_candidate):
                continue
        ...
        for filename in sorted(filenames):
            ...
            if is_excluded_path(relative_path):
                continue
            if not is_safe_markdown_file(path, root):
                continue
            try:
                prefix = path.read_text(encoding="utf-8")[:4096]
            except OSError:
                continue
            if GENERATED_PACKET_MARKER in prefix:
                continue
...
def build_records_for_file(path: Path, root: Path) -> list[SectionRecord]:
    ...
    if GENERATED_PACKET_MARKER in text[:4096]:
        return []
